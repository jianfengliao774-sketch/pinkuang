...
已证实的差异为：开挖链上窗口 1–64 块、停挖冷却超过 1200 块、Mining 已封存且 owner 为零、pending 非实时应领额。池资产余额也已变化，历史近似数值不能作当前报价。没有更改认购费率、1/4/95 分账、投票规则或 operator 权限。

建议项目方将上述事实纳入 v0.4 勘误，后续 M1 按这些链上事实实现；若仍要求沿用旧时间单位或旧 pending 等式，现有协议无法满足。原始两份需求文件保持不变，等待这项规范差异决定，不自行发明业务规则。本卡已完成所有独立验证，不存在 Q1/Q3/Q6 所列的降级或项目终止触发。页面参照“芯火夺宝”的要求已记录在 [视觉参考](design-reference.md)，本卡未提前实现 M3。
