...
for (const mode of ['recover','exhaust','not-retryable']) {
  ...
  const server=createServer((req,res)=>{let data='';req.on('data',c=>data+=c);req.on('end',()=>{
    ...
    const value=method==='eth_chainId'?'0x38':method==='eth_gasPrice'?'0x1':method==='eth_getBlockByNumber'?block:method==='eth_getBalance'||method==='eth_getTransactionCount'?'0x0':method==='eth_getCode'?'0x':method==='eth_getStorageAt'?zero32:undefined;
  ...
  const args=['test','--root',root,'--match-contract','RetryProbe','--fork-url',url,'--fork-block-number','123728000','--threads','1','--compute-units-per-second','50','--fork-retries','10','--fork-retry-backoff','2000','-vv'];
...
writeFileSync(join(root,'results.json'),JSON.stringify({fixture:'local synthetic JSON-RPC; not BSC chain evidence',forgeVersion:'1.7.1',results},null,2)+'\n');
