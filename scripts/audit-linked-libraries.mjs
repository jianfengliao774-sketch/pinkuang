...
const expectedLibraries = ['MiningOperations', 'PurchaseValidation', 'RewardAccounting', 'SaleGovernance', 'ShareCheckpoints'];
...
function vaultLinks(bytecode, label) {
  ...
  assert.deepEqual(links.map(link => link.name).sort(), expectedLibraries,
    `${label} must link exactly MiningOperations, PurchaseValidation, RewardAccounting, SaleGovernance and ShareCheckpoints`);
  return links;
}
...
// deliberately the five production library source units, not a general security audit.
...
export default function auditLinkedLibraries(root, logRoot) {
  ...
  console.log('PASS: PoolVault links exactly 5 reviewed libraries; source hashes, unlinked templates and scoped AST gates recorded.');
  return audit;
}
