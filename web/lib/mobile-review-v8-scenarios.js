...
const main='01 · 主页面';
add('A01','拼矿总览 · 完整首页',main,'首页 / #home',{route:'home'},'包括首屏、初衷愿景、平台统计、项目入口、权益说明和底部宣传语。');
...
add('A05','矿机转让 · 份额交易',main,'矿机转让 → 份额交易',{route:'market',connected:true});
add('A06','矿机转让 · 整机出售',main,'矿机转让 → 整机出售',{route:'market',marketTab:'整机出售',connected:true});
...
for(const [i,tab] of ['全部记录','收益分配','收益领取'].entries())add(`A${String(9+i).padStart(2,'0')}`,`公开记录 · ${tab}`,main,`公开记录 → ${tab}`,{route:'records',recordFilter:tab});
...
for(const [i,[sort,label]] of ['id-asc','price-asc','price-desc','funded-desc','daily-desc','unit-asc'].map(key=>SORT_OPTIONS.find(x=>x[0]===key)).entries())add(`B${10+i}`,`排序 · ${label}`,catalog,`募集中 → 排序 → ${label}`,{route:'pools',filter:'募集中',catalog:{sort}},'同一列表的排序结果；六个排序选项均列入审查。');
const detail='03 · 六台矿机详情';
let d=0;
for(const p of pools)for(const tab of ['资产详情','收益记录','共同决策','参与者'])add(`C${String(++d).padStart(2,'0')}`,`${p.name} #${p.id} · ${tab}`,detail,`#detail/${p.id} → ${tab}`,{route:'detail',poolId:p.id,detailTab:tab,connected:true},tab==='收益记录'?'按当前矿机筛选收益记录；无记录时显示空态。':tab==='共同决策'?'仅 #8204 有进行中的演示提案，其余显示暂无提案。':'');
const dialogs='04 · 弹窗';
...
[
 ...
 ['买入矿机份额','矿机转让 → #8204买入',{route:'market',connected:true,qty:1,modal:modal('buy',{order:orders[1]})}],
 ['出售我的份额','矿机转让 → 出售我的份额',{route:'market',connected:true,modal:modal('sell')}],
 ['购买整台矿机','矿机转让 → 整机出售 → 购买整台矿机',{route:'market',marketTab:'整机出售',connected:true,modal:modal('whole')}],
...
const funded=pools.map(p=>p.id==='17006'?{...p,funded:100,shares:9,cost:.43632,members:22,status:'Funded'}:p);
add('E01','认购完成 · 待购机详情',states,'#17006 → 认购最大9份 → 确认',{route:'detail',poolId:'17006',connected:true,pools:funded,toast:'演示认购成功：{count} 份',toastParams:{count:9}},'满募后等待购机，时间线停留在完成购机步骤。');
...
add('E08','矿机转让 · 我的挂单有数据',states,'出售我的份额 → 创建演示挂单',{route:'market',listed:{pool:'16210',shares:5,price:.068},connected:true,toast:'演示挂单已创建'});
add('E09','矿机转让 · 撤销后空态',states,'我的挂单 → 撤销挂单',{route:'market',connected:true,toast:'演示挂单已撤销，份额已解锁'});
...
add('E19','手机导航 · 展开',states,'手机顶部 → 打开导航',{route:'market',menu:true},'手机侧栏抽屉与遮罩；打开单页可测试关闭和导航。');
const mobile='06 · 手机补充场景';
add('F01','从参与者转让 · 指定 #8204',mobile,'#8204 → 参与者 → 转让份额',{route:'detail',poolId:'8204',detailTab:'参与者',connected:true,modal:modal('sell',{sellPoolId:'8204'})});
add('F02','份额挂单 · #8204',mobile,'选择 #8204 → 创建挂单',{route:'market',connected:true,listed:{pool:'8204',shares:5,price:.086}});
add('F03','矿机详情 · 未连接钱包',mobile,'直接打开 #16928',{route:'detail',poolId:'16928'});
add('F04','手机导航 · 已连接账户',mobile,'连接钱包后 → 打开导航',{route:'overview',connected:true,menu:true});
...
export const reviewGaps=[
 ...
 {title:'演示日期与数据',detail:'项目金额、人数及截止时间为演示值；首页币价读取实时参考价缓存，可能因网络显示暂不可用。'},
