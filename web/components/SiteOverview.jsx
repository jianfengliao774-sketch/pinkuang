...
export default function SiteOverview({pools,onExplore,onAccount,onRules,onRecords}){
 ...
 return <div className="bemine-home">
  ...
  <section className="bemine-why"><div><span className="bemine-kicker">YOUR SHARE. YOUR VOICE.</span><h2>{t('一起拼矿，')}<br/>{t('也一起决定。')}</h2><p>{t('资产有归属，产出有记录，')}<br/>{t('重大事项由共同持有人参与决定。')}</p><button className="text-button" onClick={onRules}>{t('了解平台规则')}<ArrowRight size={15}/></button></div><div className="bemine-benefits">{[[Layers3,'从 1 份开始','每台矿机共有100份额，每个地址可选择持有数量，至多49份。'],[Activity,'按份分享真实产出','产出先归集到池子，个人按份领取；已入账收益永久保留。'],[Vote,'共同参与重要决策','地址数须过半；折价出售需至少 60 份赞成，其余需超过 50 份。'],[ShieldCheck,'把记录摆在明处','查看归集、分配与本人领取记录，了解每一份权益。']].map(([Icon,title,desc])=><article key={title}><Icon size={23}/><h3>{t(title)}</h3><p>{t(desc)}</p></article>)}</div></section>
