// Generated by scripts/build-review-snapshots.py. Review-only snapshot.
'use client';
...
function ProjectRows({ pools, onDetails, status }) {
  ...
  return <div className="catalog-table-scroll" role="region" aria-label={t("矿机信息列表")} tabIndex={0}>
    <table className="catalog-table"><thead><tr>
      <th>{t("矿机 / 来源")}</th><th>{t("全机日产出")}<small>{t("BEM / 天")}</small></th><th>{status === 'Listed' ? t('当前挂牌价') : status === 'Active' ? t('购入整机价') : t('目标整机价')}<small>BNB</small></th><th>{t("日产能价")}<small>{t("BNB / (BEM / 天)")}</small></th><th>{status === 'Funding' ? t('募集进度') : status === 'Listed' ? t('出售状态') : t('挖矿状态')}</th><th aria-label={t("操作")}/></tr></thead>
      <tbody>{pools.map(pool => <tr key={`${pool.series}-${pool.id}`}>
        ...
        <td className="catalog-number"><strong>{amount(projectPrice(pool))}</strong><small>{status === 'Funding' ? t('每份 {amount} BNB', { amount: amount(projectPrice(pool) == null ? null : projectPrice(pool) / 100, 3) }) : status === 'Listed' ? t('整机挂牌价格') : t('历史购入金额')}</small></td>
...
export default function PoolCatalog({ pools, onDetails, initialTab = '项目总览', reviewState = {} }) {
  const { t } = useI18n();
  const [status, setStatus] = useState(() => initialStatus(initialTab));
  useEffect(() => { setStatus(initialStatus(initialTab)); }, [initialTab]);
  const [query, setQuery] = useState(reviewState.query||'');
  const [filters, setFilters] = useState({ ...DEFAULT_FILTERS, ...reviewState.filters });
  const [sort, setSort] = useState(reviewState.sort||'id-asc');
  const [filterOpen, setFilterOpen] = useState(reviewState.filterOpen||false);
  ...
  return <section className="bem-catalog">
    ...
    </div><div className="catalog-filter-foot"><p>{invalidRange ? t('最低值不能大于最高值，请调整筛选范围。') : t('同一矿机可能同时出现在两个市场，列表仅展示一次。')}</p><button onClick={() => { setFilters({ ...DEFAULT_FILTERS }); setQuery(''); }}>{t("重置筛选")}</button></div></div>}
    ...
    <div className="catalog-notes"><Info size={17}/><div><p>{t("当前展示演示项目与演示来源标签，尚未接入 TapeOut 官方及 Firsto 实时市场。")}</p><p>{t("日产能价 = 对应整机价格 ÷ 全机估计日产出，单位为 BNB / (BEM / 天)，不代表回本时间。")}</p></div></div>
  </section>;
}
