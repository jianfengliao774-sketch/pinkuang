  ...
  return <div className="catalog-table-scroll" role="region" aria-label={t("矿机信息列表")} tabIndex={0}>
    <table className="catalog-table"><thead><tr>
      ...
      <tbody>{pools.map(pool => <tr key={`${pool.series}-${pool.id}`}>
        ...
        <td className="catalog-number"><strong>{amount(status === 'Funding' ? purchaseTotal(pool) : projectPrice(pool), status === 'Listed' ? 2 : 3)}</strong><small>{status === 'Funding' ? t('每份 {amount} BNB', { amount: amount(subscriptionPrice(pool), 5) }) : status === 'Listed' ? t('整机挂牌价格') : t('历史购入金额 · 含买方费用')}</small></td>
...
export default function PoolCatalog({ pools, onDetails, initialTab = '项目总览' }) {
  ...
  return <section className="bem-catalog">
    ...
    <div className="catalog-notes"><Info size={17}/><div><p>{t("当前展示演示项目与演示来源标签，尚未接入 TapeOut 官方及 Firsto 实时市场。")}</p><p>{t("目标募集金额、每份认购金额及挖矿中的购入金额均含商城 1% 买方手续费。价格筛选和日产能价：募集中按商城标价、挖矿中按含费购入金额、整机出售中按当前挂牌价计算。")}</p><p>{t("日产能价 = 对应整机价格 ÷ 全机估计日产出，单位为 BNB / (BEM / 天)，不代表回本时间。")}</p></div></div>
  </section>;
}
