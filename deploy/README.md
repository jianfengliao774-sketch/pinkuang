...
以[页面合约优化报告](../docs/audits/2026-09-26/frontend-contracts.md)为最新覆盖：取消业务销毁；收益、份额成交、整机出售平台费均为 1%，其余归权益持有人；份额表决期间冻结交易，订单 7 天到期。Factory 自动部署只读 Lens，最终核验检查其归属和运行代码，钱包交易仍为 13 笔。旧测试日志保留作历史。Vite 启动/构建独立重编源码，产物未同步时拒绝构建，需先运行 `npm run artifacts`。
